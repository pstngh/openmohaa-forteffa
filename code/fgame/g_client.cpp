...
void G_BotConnect(int clientNum, qboolean firstTime, const char *userinfo)
{
    ...
    // Read the session data from a clean client structure.
    ...
    // Every bot owns a real server-side client slot so master-server and
    // player queries count it exactly like a human player.
