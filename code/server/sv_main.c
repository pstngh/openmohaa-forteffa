...
void SV_BotConnect(int clientNum, const char *userinfo) {
    ...
    // A free slot can retain state from an earlier occupant. Initialize bots
    // exactly once from a clean client structure, just like DirectConnect.
...
const char *SV_GetClientStatusAddress(const client_t *cl, int *botPortIndex) {
    if (cl->netchan.remoteAddress.type == NA_BOT) {
        const int port = 12203 + (*botPortIndex % 5);
